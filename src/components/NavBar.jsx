...
export const NavBar = () => {
    ...
    const handlerScroll = () => {
      ...
      if(navbarRef.current){
        if(scrollTop > lastScroll){
          ...
          if(scrollTop == 0 ){
            navbarRef.current.style.transition = "background-color 0.3s ease-in-out"
  ...
  return (
    ...
    <header ref={navbarRef} className="flex flex-row fixed top-0 w-screen justify-between h-14 px-10 text-white items-center transition-transform duration-200 ease-in-out">
      ...
      <nav className="flex basis-0 flex-grow justify-end">
        <ul className="flex [&>li>svg]:h-[24px] [&>li]:w-9 [&>li>svg]:mr-1">
          <li>
